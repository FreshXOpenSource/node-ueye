...
Camera.prototype.nextEvent = function (evt, name) {
    ...
    co (function*() {
        try {
            yield self.waitEvent(evt, 50000);
            self.emit(name);
        } finally { 
            setTimeout(() => {
...
Camera.prototype.nextImage = function (handler, done) {
    ...
    co(function* () {
        yield self.waitEvent(defines.IS_SET_EVENT_FRAME, 50000);
...
Camera.prototype.getArbitraryAOISupported = function () {
    ...
    return new Promise((resolve, reject) => {
        ...
        (err, nRet) => {
            ...
            return resolve(nRet !== defines.IS_SUCCESS || nAOISupported != 0); 
        });
    });
};
