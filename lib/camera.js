...
Camera.prototype.memoryFreezeVideo = function (sequence, timeout) {
    const self = this;

    return new Promise((resolve, reject) => {
        return binding.is_MemoryFreezeVideo.async(self.hCam, self.mem[sequence].id, timeout, (err, nRet) => {
            if (err || nRet !== defines.IS_SUCCESS) {
                return reject(new Error(`is_MemoryFreezeMode: ${err || nRet}`));
            }
...
Camera.prototype.setMemoryMode = function (mode, timeout) {
    const self = this;

    return new Promise((resolve, reject) => {
        return binding.is_SetMemoryMode.async(self.hCam, mode, timeout, (err, nRet) => {
            if (err || nRet !== defines.IS_SUCCESS) {
                return reject(new Error(`is_SetMemoryMode: ${err || nRet}`));
            }
...
Camera.prototype.setExternalTrigger = function (flags) {
    const self = this;

    return new Promise((resolve, reject) => {
        return binding.is_SetExternalTrigger.async(self.hCam, flags, (err, nRet) => {
            if (err || nRet !== defines.IS_SUCCESS) {
                return reject(new Error(`is_SetExternalTrigger: ${err || nRet}`));
            }
